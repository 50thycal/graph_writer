...
describe("canvas adapter", () => {
  it("round-trips semantic metadata and changed geometry", () => {
    const document = createStudioDocument({ id: "doc-1", name: "Round trip", mode: "graph", elements: [{ id: "agent-1", type: "agent", name: "Researcher", transform: { x: 10, y: 20, width: 240, height: 120 }, properties: { retries: 2 }, intent: ["Gather evidence."] }] });
